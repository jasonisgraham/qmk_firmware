...
[_BASE] = LAYOUT_planck_grid(
                             top_left,    my_q,    my_w,    my_e,    my_r,    my_t,     my_y,           my_u,    my_i,    my_o,     my_p, backspace,
                             my_lctl , my_a,   my_s,   my_d,   my_f,   my_g,   my_h,         my_j,   my_k,   my_l,   my_semicolon, KC_QUOTE,
                             my_left_shift,        my_z,           my_x,           my_c,           my_v,           my_b,  my_n,   my_m,   my_comma,   my_period,   my_forward_slash, KC_BSLASH,
                             hyper, super_meta_hyper , all_mods , alt, RAISE,  super,  my_space,           LOWER,          KC_LBRACKET,  KC_DOWN, key_4_11,  key_4_12
                              ),
...
[_LOWER] = LAYOUT_planck_grid(
                              ...
                              _______, KC_F5,          KC_F6,          KC_F7,          KC_F8,          my_lower_g,     KC_LEFT,        my_lower_j,        my_lower_k,          my_lower_l,       my_lower_semi, _______,
                              _______,   KC_F9,         KC_F10,         KC_F11,         KC_F12,         KC_APPLICATION,   my_lower_n, my_lower_m, my_lower_comma,   my_lower_period, my_lower_slash, KC_ENTER,
                              SYSTEM_LAYER_ACTIVATE, _______, _______ , alt,   MO(_EDITING), MO(_MOUSE),                    MO(_EXTENDED), lower_LOWER,          lower_right_of_lower, lower_key_4_11, lower_key_4_12, lower_right_of_super


                              ),

[_RAISE] = LAYOUT_planck_grid(
                              LALT(KC_D),    _______,          my_raise_w,  KC_END,         EMACS_BUFFER_REVERT,       _______,            my_raise_y, my_raise_u,           my_raise_i,           my_raise_o,           my_raise_p, KC_DEL,
                              _______, my_raise_a, my_raise_s,   my_raise_d, my_raise_f,  OSL(_MACROS),        my_raise_h,         my_raise_j,           my_raise_k,           my_raise_l,           raise_semi, _______,
                              _______, _______, _______,_______,   _______,          _______,      my_raise_n,           my_raise_m,           my_raise_comma,         KC_3,           raise_slash, KC_INSERT,
                              SYSTEM_LAYER_ACTIVATE, _______, _______,        _______,        _______, _______,        my_space,   raise_key_4_9,       KC_0,  _______,         TO(_BASE), lower_right_of_super
...
[_ALL_MODS] = LAYOUT_planck_grid(
                                 ...
                                 _______, _______, _______, _______, _______, _______,   _______, _______, _______, _______, _______, _______,
                             _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
                                 _______, _______, EMACS_FRAME_FULL_SCREEN, _______,    _______, _______,      MO(_MOUSE), _______, _______, _______, _______, _______
...
[_WINDOWS] = LAYOUT_planck_grid(
                                _______, WINDOWS_Q,   WINDOWS_W,   LGUI(KC_E),     TD(DANCE_40),   LGUI(KC_T),  LGUI(KC_Y),   LGUI(KC_7),        LGUI(KC_8),     LGUI(KC_9),     LGUI(KC_0), LGUI(KC_MINUS),
                                RALT(RGUI(KC_ESCAPE)), LGUI(KC_A),  select_slack, show_desktop     ,LGUI(KC_F),      _______,   LGUI(KC_H),     TD(DANCE_43),   windows_k,   windows_l,   _______, TD(DANCE_42),
                                TO(_BASE), LGUI(KC_Z),     LGUI(KC_X),     LGUI(KC_C),     RGUI(KC_V),    _______, WINDOWS_N,   LGUI(KC_M),  _______, RSFT(LGUI(KC_K)), RCTL(RGUI(KC_Q)),  LGUI(KC_MINUS),
                                _______, _______, _______,         _______,        _______,      ALT_TAB , LALT(KC_8),      _______,  ALT_TAB, _______, _______, _______
                                ),

// 7
[_MACROS] = LAYOUT_planck_grid(
                               _______,   _______, _______, _______, emacs_r, _______,  EMACS_COPY_FILE_PATH, EMACS_BACKWARD_UP, _______, _______, _______, EMACS_GOTO_GET_FEED,
                               _______, emacs_a, _______, EMACS_YAS_DOC,     emacs_f,      EMACS_GOTO_PARSE_ITEM,  _______, _______, EMACS_YAS_KEYS_DESCRUCTURE, EMACS_YAS_TL,     EMACS_COMMENT_READER, EMACS_INSERT_GET_FEED,
...
[_SHIFTLOCK] = LAYOUT_planck_grid(
                                  ...
                                  _______, _______, _______, _______, _______, _______,        _______,    LOWER,  _______, _______, _______, KC_NO
                                  ),

// 9
[_WINMOVE] = LAYOUT_planck_grid(
                                _______,   _______, _______, _______, _______, _______,   RGUI(RSFT(KC_Y)),_______, _______, RGUI(RSFT(KC_O)),_______, WINDOW_ALWAYS_ON_TOP,
                                _______, _______, _______, LALT(LGUI(LCTL(RCTL(KC_D)))),_______, _______,  winmove_tile_left,   winmove_tile_down, winmove_tile_up ,   winmove_tile_right,   _______, _______,
                                _______, _______, _______, EMACS_FRAME_FULL_SCREEN, _______, _______,  RGUI(RSFT(KC_N)), winmove_mon_up, _______, RGUI(RSFT(KC_DOT)),_______,  _______,
                                _______, _______, _______, _______, TO(0),          _______,          winmove_mon_left,   winmove_mon_down,   winmove_mon_right,    _______, _______,  _______
                                ),
...
_______, EMACS_DESC_KEY, LCTL(KC_7),     LCTL(KC_E), EMACS_HELM_MARK_RINGS,     EMACS_TRANSPOSE,         EMACS_YANK_IN_SEXP  , scroll_next,     scroll_prev, EMACS_HELM_OCCUR, EMACS_PROJECTILE_FIND_FILE, KC_DELETE,
                                _______, _______, EMACS_LOCCUR, EMACS_KILL_IN_SEXP, RALT(KC_ENTER),                EMACS_SCROLL_UP, RALT(RSFT(KC_H)), editing_j, editing_k, editing_l, EMACS_COMMENT_READER, _______,
...
[_MOUSE] = LAYOUT_planck_grid(
                              ...
                              _______, _______, _______, _______, _______, _______,   LALT(LGUI(KC_N)), KC_MS_BTN3,     TD(DANCE_35),   TD(DANCE_36),   TO(_ALL_MODS),  _______,
...
[_BRIGHTNESS] = LAYOUT_planck_grid(
                                   _______, _______, _______, _______, _______, _______,  _______, _______, APL_IOTA_UNDERBAR, _______, _______,    _______,
