#define planck_base   mod_r1, top_left,    my_q,    my_w,    my_e,    my_r,   my_t,    my_y,           my_u,    my_i,    my_o,     my_p, \
    ...
    mod_r4, hyper, apl_level3_and_adhoc_hotkey, all_mods,      alt, RAISE,  super,   my_space,           LOWER,          emacs_and_backspace,   key_4_11,  key_4_12
...
#define planck_raise  _______, LSFT(KC_TAB),    my_raise_q,          my_raise_w,  KC_END,         EMACS_BUFFER_REVERT,       my_raise_t, my_raise_y, my_raise_u,           my_raise_i,           my_raise_o,           my_raise_p, \
    raise_backspace, _______, my_raise_a, my_raise_s,   my_raise_d, my_raise_f,  TD(DANCE_MACROS),        my_raise_h,         my_raise_j,           my_raise_k,           my_raise_l,       raise_semi, \
    ...
    OSL(_HYPER), MO(_SYSTEM), _______, KC_ENTER,        _______,        TO(_BASE), _______,      my_space,   raise_key_4_9,       raise_backspace,  _______,         TO(_BASE)
...
#define planck_emacs                                    _______, EMACS_TOGGLE_REPL, EMACS_DESC_KEY, _______, _______, EMACS_HELM_MARK_RINGS, EMACS_TRANSPOSE, EMACS_HELM_KILL_RINGS, EMACS_DEFUN_END, EMACS_DEFUN_BEGIN, _______, _______, \
    _______, _______, _______, REFRAME_SUBSCRIBE, REFRAME_DISPATCH, EMACS_FINDER_COMMENTARY, _______,    EMACS_SPLIT_WINDOW_LEFT, EMACS_SPLIT_WINDOW_DOWN, EMACS_SPLIT_WINDOW_UP, EMACS_SPLIT_WINDOW_RIGHT,_______, \
...
#define planck_editing         LALT(KC_UNDS), EMACS_ACE_WINDOW_SELECT, EMACS_DESC_KEY, LCTL(KC_7),     LCTL(KC_E), _______, EMACS_EVIL_LISP_STATE, EMACS_YANK_IN_SEXP  , EMACS_NEXT_SEXP ,EMACS_PREV_SEXP ,  EMACS_HELM_OCCUR, PAREDIT_BACKWARD_UP, \
...
#define planck_windows LGUI(KC_MINUS), ALT_TAB, WINDOWS_Q,   LGUI(KC_W),   WINDOWS_E,     TD(DANCE_40),   LGUI(KC_T),   LGUI(KC_Y),   LGUI(KC_7),        LGUI(KC_8),     LGUI(KC_9),     LGUI(KC_0), \
    _______, LALT(LGUI(KC_ESCAPE)), LGUI(KC_A),  select_slack, show_desktop     ,LGUI(KC_F),      _______,   LGUI(KC_H),     TD(DANCE_43),   windows_k,   windows_l,   LGUI(KC_SCLN), \
...
#define planck_macros                                 EMACS_GOTO_GET_FEED, TO_BASE,   _______, _______, _______, emacs_r, COPY_TEXT_OPEN_NEW_TAB_SEARCH, EMACS_COPY_FILE_PATH, EMACS_BACKWARD_UP, EMACS_INSERT_GI_GET_FEED, CD_C9, COPY_LATEST_FILE_TO_CLIPBOARD, \
